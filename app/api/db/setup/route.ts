...
export async function GET() {
  try {
    // Sahi schema — lead_intelligence
    const tableResult = await pool.query(`
      SELECT column_name, data_type, is_nullable
      FROM information_schema.columns
      WHERE table_schema = 'lead_intelligence'
        AND table_name = 'company_master'
      ORDER BY ordinal_position
    `);
    ...
    const stagingCount = await pool.query(`
      SELECT COUNT(*) as total_rows 
      FROM lead_intelligence.staging_businesses
    `);
    ...
    return NextResponse.json({
      ...
      columns: tableResult.rows,
      counts: {
        raw_records: rawCount.rows[0]?.total_rows || 0,
        staging_businesses: stagingCount.rows[0]?.total_rows || 0,
        company_master: countResult.rows[0]?.total_rows || 0,
      }
    });
  } catch (err: any) {
    return NextResponse.json({ success: false, error: err.message }, { status: 500 });
